...
class SheetA4:
    def __init__(self, format, DPI=300, bg_color=(255, 255, 255), sheet_format="A4"):
        ...
        if format == "Standard H":
            self.sheet = Image.new("RGB", (int(210 * self.DPM), int(297 * self.DPM)), self.bg_color) #vertical sheet
        else:
            self.sheet = Image.new("RGB", (int(297 * self.DPM), int(210 * self.DPM)), self.bg_color) #horizontal sheet
